...
export default async function AnimalDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: animal }, { data: weights }, { data: farm }] = await Promise.all([
    supabase.from("animals").select("*").eq("id", id).single(),
    supabase.from("animal_weights").select("*").eq("animal_id", id).order("weighed_at"),
    supabase.from("farms").select("*").eq("id", membership.farm_id).single(),
  ]);
  ...
  return (
    <main className="mx-auto max-w-5xl space-y-4 px-4 py-6">
      <Card>
        ...
        <CardDescription>
          Chip: {animal.chip_id || "-"} | Arete: {animal.ear_tag || "-"} | Estado: {animal.status}
        </CardDescription>
      ...
      <EventManager farmId={membership.farm_id} animalId={id} userId={user.id} />

      <Link href={`/animal/${id}/genealogia`} className="inline-block rounded-xl bg-slate-200 px-4 py-3 font-semibold">
        Ver genealogía
      </Link>
    </main>
  );
