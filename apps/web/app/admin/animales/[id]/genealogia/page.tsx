...
export default async function AdminGenealogyPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const { supabase } = await requireMembership("supervisor");
  const { data: animal } = await supabase.from("animals").select("farm_id").eq("id", id).single();

  if (!animal) return <p>No existe este animal.</p>;
  await requireMembershipForFarm(animal.farm_id, "supervisor");
