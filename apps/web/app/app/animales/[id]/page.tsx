...
export default async function AppAnimalDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const { supabase: bootstrap, user } = await requireMembership();

  const { data: animal } = await bootstrap.from("animals").select("*").eq("id", id).single();

  if (!animal) return <p>No existe este animal.</p>;
  const { supabase } = await requireMembershipForFarm(animal.farm_id);
  ...
  return (
    <main className="mx-auto max-w-5xl space-y-4 px-4 py-6">
      ...
      <Link
        href={`/app/animales/${id}/genealogia`}
